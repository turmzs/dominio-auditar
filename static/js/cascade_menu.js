/* ============================================
   CASCADE MENU CONTROLLER - DOMINIO ESCRITA FISCAL
   ============================================ */
...
function buildCascadeMenu() {
    ...
    menuStructure.forEach(section => {
        const dropdown = createMenuDropdown(section);
 ...
 */
function createMenuDropdown(section) {
    ...
    if (section.children) {
        ...
        link.className = 'menu-item has-submenu';
        link.href = '#';
        ...
        section.children.forEach(child => {
            ...
            if (child.children) {
                li.innerHTML = createNestedSubmenu(child);
            } else {
                li.innerHTML = `<a href="#" onclick="selectItem(event, '${child.id}')">${child.label}</a>`;
            }
            menu.appendChild(li);
        });
 ...
 */
function createNestedSubmenu(node) {
...
// Initialize
document.addEventListener('DOMContentLoaded', buildCascadeMenu);
